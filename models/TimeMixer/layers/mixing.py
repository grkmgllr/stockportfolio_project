...
class MultiScaleSeasonMixing(nn.Module):
    ...
    Expects season_list items shaped [B,C,T]
    Returns list shaped [B,T,C]
    """
    ...
    def forward(self, season_list):
        out_high = season_list[0]   # [B,C,T0]
        out_low = season_list[1]    # [B,C,T1]
        out_season_list = [out_high.permute(0, 2, 1)]  # [B,T,C]

        for i in range(len(season_list) - 1):
            out_low_res = self.down_sampling_layers[i](out_high)  # [B,C,T_{i+1}]
...
class MultiScaleTrendMixing(nn.Module):
    ...
    Expects trend_list items shaped [B,C,T]
    Returns list shaped [B,T,C]
    """
    def __init__(self, seq_len: int, down_window: int, down_layers: int):
        super().__init__()
